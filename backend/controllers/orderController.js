...
exports.getAllOrders = async function (req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Orders endpoint failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
...
exports.createOrder = async function (req, res) {
  ...
  try {
    ...
    if (prezzo == null)
      return res.status(400).json({ message: "prezzo è obbligatorio" });
    const created = await orderDAO.createOrder(conn, {
      user_id,
      prezzo,
      payment_provider,
      payment_ref,
    ...
    if (!created)
      return res.status(500).json({ message: "Creazione ordine fallita" });
...
exports.confirmPayment = async function (req, res) {
  ...
  try {
    ...
    if (!id_ordine)
      return res.status(400).json({ message: "id_ordine obbligatorio" });
    // Only update payment_status and payment_ref, do NOT touch stato
    const updated = await orderDAO.markOrderPaid(conn, id_ordine, {
      payment_ref,
    ...
    console.log(
      "[DEBUG] confirmPayment called for order=%s itemsProvided=%s",
      id_ordine,
    ...
    if (typeof orderDAO.processOrderItemsAndUpdateProducts === "function") {
      const ok = await orderDAO.processOrderItemsAndUpdateProducts(
        conn,
        id_ordine,
        items
      );
      console.log(
        "[DEBUG] processOrderItemsAndUpdateProducts result for order=%s => %o",
        id_ordine,
        ok
      );
      if (!ok)
        console.warn("Failed processing order items for order", id_ordine);
    } else {
      // fallback: original behaviour
      if (items && Array.isArray(items) && items.length > 0) {
        const ok = await orderDAO.insertOrderItems(conn, id_ordine, items);
        console.log(
          "[DEBUG] insertOrderItems fallback result for order=%s => %o",
          id_ordine,
          ok
        );
      } else {
        ...
        console.log(
          "[DEBUG] updateProductsFromOrderItems fallback result for order=%s => %o",
          id_ordine,
          ok
        );
      }
    ...
    res
      .status(500)
      .json({ message: "Confirm payment failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
...
exports.getPendingOrders = async function (req, res) {
  ...
  try {
    ...
    res.status(500).json({
      message: "Pending orders endpoint failed",
      error: error.message,
    });
  } finally {
    if (conn) conn.done();
...
exports.cancelOrder = async function (req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Cancel order failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
...
exports.updateDeliveryData = async function (req, res) {
  ...
  try {
    ...
    console.log(
      "[DEBUG] updateDeliveryData controller received orderId=%s deliveryData=%o",
      orderId,
      deliveryData
    );
    ...
    const updated = await orderDAO.updateDeliveryData(
      conn,
      orderId,
      deliveryData
    );
    ...
    res
      .status(500)
      .json({ message: "Update delivery data failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
...
exports.getOrderTracking = async function (req, res) {
  ...
  try {
    ...
    if (!orderId)
      return res.status(400).json({ message: "order id non valido" });
    conn = await db.getConnection();
    // verify ownership
    ...
    console.log(
      "[DEBUG] getOrderTracking: verifying order ownership ok for orderId=%s userId=%s",
      orderId,
      user_id
    );
    const spediDAO = require("../DAO/spediDAO");
    ...
    console.log(
      "[DEBUG] getOrderTracking: events fetched for orderId=%s count=%s",
      orderId,
      Array.isArray(events) ? events.length : "non-array"
    );
    if (Array.isArray(events) && events.length > 0)
      console.log("[DEBUG] getOrderTracking: first event=%o", events[0]);
    res.json({ tracking: events });
  } catch (error) {
    console.error("controller/orderController.js getOrderTracking", error);
    res
      .status(500)
      .json({ message: "Get order tracking failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
...
exports.getOrdersByUser = async function (req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Get user orders failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
...
exports.getOrderDetails = async function (req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Get order details failed", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};

exports.advanceOrderStatus = async function (req, res) {
  let conn;
  const id = Number(req.params.id);
  if (!id) return res.status(400).json({ message: "Invalid order id" });
  ...
  try {
    ...
    // Trova lo stato attuale
    let currentStatus = "pending";
    if (events && events.length > 0) {
      const latest = events.reduce((a, b) =>
        new Date(a.data_evento) > new Date(b.data_evento) ? a : b
      );
      currentStatus = (latest.stato_pacco || "pending").toLowerCase();
    }

    // Trova il prossimo stato
    const currentIndex = statusOrder.findIndex((s) =>
      currentStatus.includes(s)
    );
    if (currentIndex === -1 || currentIndex >= statusOrder.length - 1) {
      return res.status(400).json({ message: "Impossibile avanzare lo stato" });
    }

    const nextStatus = statusOrder[currentIndex + 1];

    // Recupera la località dal body della richiesta
    const { localita } = req.body;
    const location = localita || "Centro logistico";

    // Inserisci nuovo evento con località personalizzata
    await conn.none(
      "INSERT INTO tracking (id_ordine, data_evento, stato_pacco, localita, note) VALUES ($1, NOW(), $2, $3, $4)",
      [id, nextStatus, location, '']
    );
    ...
    console.error("advanceOrderStatus error:", error);
    res
      .status(500)
      .json({ message: "Errore aggiornamento stato", error: error.message });
  } finally {
    if (conn) conn.done();
  }
};
